...
for i =1:1
    ga =  cla_gradient1(labelPath,FC_raw,n_sub,246,savePath);
    aa(i,:)=ga;
end
...
function [grad1] = cla_gradient1(labelPath, FC_raw, numSub,numROI, savePath)   
    ...
    for siteNO=1:siteNum
        fc_avg_site(siteNO,:,:) = squeeze(mean(FC_raw{siteNO,1},3));
    end
    fc_avg = squeeze(mean(fc_avg_site,1));
    ...
    for siteNO=1:siteNum
        for subType=1:subTypeNum 
            ...
            for subNO=1:numSub(siteNO,subType) 
                data_FC1{subNO+1} = FC_raw{siteNO,subType}(:,:,subNO);
            end
            %运行模型，fit将处理cell元组的每个元素（即每个被试）路径中包含spm12会报错，可能是某些函数重名
            Gp{siteNO,subType}=Gp_model.fit(data_FC1);
            for subNO=1:numSub(siteNO,subType) 
                
                grad{siteNO,subType}(:,subNO)=Gp{siteNO,subType}.aligned{subNO+1}(:,gradNO);
            end
