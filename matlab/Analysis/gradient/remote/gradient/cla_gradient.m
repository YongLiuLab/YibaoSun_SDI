...
load('F:\Code/coupling/matlab/file/raw/sub_info.mat')
%load('/home/syb/Code/coupling/matlab/file/raw/RawLoad.mat')

labelPath = 'F:\Code\coupling\matlab\z_source\gradient\label\mesh.gii';
savePath = 'F:\Code\coupling\matlab\utils\Analysis\gradient\remote\gradient\remote\plot';

load('F:\Code\coupling\matlab\utils\Analysis\gradient\remote\gradient\plot\GP_FC_246\Grad1.mat');
%cla_gradient1(labelPath,FC_raw,n_sub,savePath);

PlotGradient(labelPath, n_sub,savePath)

function PlotGradient(labelPath, numSub,savePath)
    %% 3、梯度可视化
    %加载conte69大脑标准空间
    ...
    for siteNO=1:siteNum
        ...
        Name=['Site' num2str(siteNO) ' 210parcel' ];
        path_fig = [savePath '/plot_210parcel_grad/' Name '.png']
        a ='F:/a.png'
        %saveas(h{siteNO}.figure,a);
    end
...
function cla_gradient1(labelPath, FC_raw, numSub,savePath)   
    ...
    Gp_model=GradientMaps('kernel','na','approach','dm','alignment','ja');    %kernal：Normalized Angle    Dimension Reduction：Diffusion Embedding
